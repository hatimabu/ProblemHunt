...
-- Get user's primary wallet for a specific chain
...
-- Mark an order as paid (called by Edge Function with service role)
...
BEGIN
    ...
    IF order_record.status != 'pending' THEN
        RAISE EXCEPTION 'Order % is not pending (status: %)', p_order_id, order_record.status;
    END IF;
