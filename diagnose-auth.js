...
const REPORT_PATH = path.resolve(process.cwd(), 'diagnosis_report.txt');
const JWKS_PATH = path.resolve(process.cwd(), 'jwks.json');
const JWKS_ERROR_PATH = path.resolve(process.cwd(), 'jwks_error.txt');
const JWT_DECODED_PATH = path.resolve(process.cwd(), 'jwt_decoded.json');
const AUTH_USER_RESPONSE_PATH = path.resolve(process.cwd(), 'auth_user_response.txt');
...
function getApiKey() {
  ...
  // also check .env and local.settings if present
  try {
