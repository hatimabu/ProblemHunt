...
try{
 ...
 await m.goto(base+'/moderation');const review=m.getByRole('article').filter({has:m.getByRole('heading',{name:reason,exact:true})});try{await review.waitFor();}catch(e){console.log('Moderator diagnostic',JSON.stringify({path:new URL(m.url()).pathname,articles:await m.getByRole('article').count(),status:await m.getByRole('status').allTextContents(),errors:await m.getByRole('alert').allTextContents(),denied:await m.getByText('You do not have moderator permission.').count()}));throw e;}
